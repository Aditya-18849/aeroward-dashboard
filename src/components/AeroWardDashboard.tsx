...
import {
    ...
    AlertTriangle,
    Eye,
    Smartphone, // FIX-4: Citizen app preview modal icon
    Zap, // ── SURGERY 2: Dispatch button icon repair ──
    CheckCircle2, // ── SURGERY 2: Dispatch button icon repair ──
...
function AqiBadge({ aqi, size = "sm", isFlashing = false }: { aqi: number; size?: "sm" | "lg", isFlashing?: boolean }) {
    ...
    return (
        <span
            // ── SURGERY 1: WARD ROW FLEXBOX RECONSTRUCTION ──
...
/** SparkLine — tiny inline SVG path of last 8 values */
function SparkLine({ data, color }: { data: number[]; color: string }) {
    ...
    return (
        <svg
            ...
            viewBox={`0 0 ${w} ${h}`}
            style={{ flexShrink: 0 }}
            aria-label="AQI trend sparkline"
        >
...
function WardRow({
    ...
    const isCritical = ward.id === "C";

    return (
        // ── POLISH 1: WARD ROW ATOMIC RECONSTRUCTION ──────────────────────────────────────
        <div
            key={ward.id}
            onClick={onToggle}
        ...
        >
            {/* SLOT 1 — Identity: ward name only, nothing else */}
            <div className="w-24 flex-shrink-0 min-w-0">
                <span className="block text-sm font-semibold text-slate-200 truncate leading-none">
                    Ward {ward.id}
                </span>
            ...
            {/* SLOT 2 — AQI badge: fixed width, never shrinks */}
            ...
            {/* SLOT 3 — Sparkline: fills remaining space */}
...
function ActionCard({
    ...
    // ── FIX 4: Citizen preview modal state ──────────────────────────
    ...
    return (
        <div
            style={{
                ...
                borderLeft: data.dispatched
                    ? `3px solid ${COLORS.nominal}`
                    : `3px solid ${pColor}`,
                overflow: "hidden",
        ...
        >
            ...
            <div style={{ padding: "10px" }}>
                ...
                >
                    {data.citizenAdvisory.map((a, i) => (
                        <div key={i}>▸ {a}</div>
                    ))}
                </div>

                {/* Buttons */}
                {/* ── POLISH 3: RIGHT SIDEBAR — DISPATCH BUTTON ICON PURGE ────────────────────────────────────── */}
                ...
                {/* ── FIX 4: Preview Citizen App Alert Button ────────────────────────── */}
                {data.priority === "HIGH" && (
                    <button
                ...
                <button
                    className="btn-ghost"
                    aria-label={`View evidence chain for ${data.wardName}`}
                    style={{
                        width: "100%",
                        padding: "6px",
                        fontSize: "11px",
            ...
            {/* ── FIX 4: Modal Implementation ────────────────────────── */}
            {showCitizenPreview && (
                <div
                    className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm"
                    onClick={(e) => {
                        e.stopPropagation();
                        setShowCitizenPreview(false);
                    }}
                >
                    <div
                    ...
                    >
                        ...
                        <div className="text-slate-400 text-sm mb-4" style={{ fontFamily: "'Inter', sans-serif" }}>
                            {data.zone} District
                        </div>
                        ...
                        <button
                            className="w-full py-2 rounded-lg text-xs font-bold tracking-wide border border-slate-600 text-slate-400 hover:border-sky-500/60 hover:text-sky-400 transition-all duration-200"
                            onClick={(e) => {
                                e.stopPropagation();
                                setShowCitizenPreview(false);
                            }}
...
function StatusBarComponent({
    wards,
    lastSyncSec,
    mounted, // FIX-3: Receiving mounted state for hydration check
}: {
    wards: WardData[];
    lastSyncSec: number;
    mounted: boolean; // FIX-3: Type definition
}) {
    // FIX-3: Undefined check (wardC?.pm10 ?? 0) and hydration guard (mounted)
    ...
    return (
        <div className="statusbar overflow-hidden">
            ...
            >
                {/* ── POLISH 2: STATUS BAR FOOTER — TOTAL LEFT SECTION REPLACEMENT ────────────────────────────────────── */}
            ...
            <div style={{ textAlign: "center", whiteSpace: "nowrap" }}>
                5 WARDS MONITORED &nbsp;·&nbsp; 47 SENSORS ACTIVE &nbsp;·&nbsp;{" "}
...
// ─── MAIN COMPONENT ───
// ── SIM 3: COUNTDOWN TIMER FORMATTER ──────────────────────────────────────
...
export default function AeroWardDashboard() {
    ...
    // ── FIX 3: Hydration guard — gate time-derived renders ──
    const [mounted, setMounted] = useState(false);

    // ── FIX 5: Auto-dispatch toggle state ──
    ...
    const [actions, setActions] = useState<ActionCardData[]>([
        {
            ...
            sourceDetail:
                "Localized construction — Site 4. Unpermitted excavation cross-referenced via sensor mesh + satellite overlay.",
            adminAction: [
                "Deploy municipal water sprinklers to Site 4.",
                "Issue stop-work order ENV-2024-0891.",
                "Halt all unpermitted digging operations immediately.",
            ],
            citizenAdvisory: [
                "Avoid outdoor exercise. Keep windows closed until 18:00.",
        ...
        },
        {
            ...
            sourceDetail:
                "Elevated NO₂ levels from Eastport Industrial Zone. Refinery stack emissions exceed permissible limits by 23%.",
            adminAction: [
                "Issue emission compliance notice to Eastport Refinery.",
                "Request EPA satellite verification pass.",
            ],
            citizenAdvisory: [
                "Sensitive groups should remain indoors.",
                "HEPA filter advisory for schools in 2km radius.",
            ],
            dispatched: false,
            dispatchTime: "",
        },
        {
            ...
            sourceDetail:
                "Moderate AQI elevation linked to peak-hour vehicular congestion on Millhaven Boulevard. Sensor cluster B-7 reporting.",
            adminAction: [
                "Activate dynamic traffic signal optimization.",
                "Divert heavy vehicles via alternate route R-14.",
            ],
            citizenAdvisory: [
                "No immediate health risk. Standard precautions advised.",
            ],
            dispatched: false,
            dispatchTime: "",
        },
    ...
    // ── FIX 3: Mount guard effect ──
    useEffect(() => { setMounted(true); }, []);

    // ── Clock + last sync (1s) & SIM 3 Countdown ──
    useEffect(() => {
        const tick = () => {
            const now = new Date();
    ...
    // Ward connections for mesh network
    const connections = [
        ["A", "B"],
        ["A", "C"],
        ["B", "D"],
        ...
        <>
            ...
            <div className="command-grid">
                ...
                <div className="topbar">
                    ...
                    {/* QA-PATCH 1: TOPBAR — GHOST TICKER PURGE */}
                    ...
                    {/* Right: System status + clock + broadcast */}
                    <div
                        style={{
                            display: "flex",
                            alignItems: "center",
                    ...
                    >
                        {/* QA-PATCH 2: TOPBAR — AUTO-DISPATCH TOGGLE CONTAINMENT */}
                        <div className="flex flex-row items-center gap-2 flex-shrink-0 relative mr-4">
                            {/* Label block — stacked, right of toggle */}
                            <div className="flex flex-col items-end flex-shrink-0">
                                <span className="text-[9px] text-slate-500 whitespace-nowrap leading-none tracking-wide uppercase">
                                    Auto-Dispatch
                                </span>
                                <span className="text-[9px] font-bold text-slate-400 whitespace-nowrap leading-none tracking-widest uppercase mt-0.5">
                                    Interventions
                                </span>
                            </div>
                            ...
                            <div
                                onClick={() => {
                                    ...
                                    if (next) {
                                        ...
                                        setTimeout(() => setShowAutoWarning(false), 3000);
                                    } else {
                                        setShowAutoWarning(false);
                                    }
                                }}
                                className={`relative w-10 h-5 rounded-full cursor-pointer transition-colors duration-200 flex-shrink-0 flex items-center px-0.5
                                    ${autoDispatch
                                        ? 'bg-red-600/80 ring-1 ring-red-500/50'
                                        : 'bg-slate-700 ring-1 ring-white/10'}`}
                                style={autoDispatch ? { boxShadow: '0 0 12px rgba(239,68,68,0.4)' } : {}}
                            >
                                <span className={`h-4 w-4 rounded-full bg-white transition-transform duration-200 inline-block flex-shrink-0
                        ...
                        <span
                            style={{
                                ...
                                boxShadow: `0 0 0 1px ${COLORS.rim}`,
                                padding: "2px 10px",
                                borderRadius: "2px",
                            }}
                        >
                        ...
                        <Radio
                            ...
                            style={{ color: COLORS.rim }}
                            aria-label="Broadcast status indicator"
                        />
                    </div>
                </div>
                    ...
                    >
                        ...
                        <div className="criticality-bar" style={{ marginTop: "8px" }}>
                            <div
                                ...
                                style={{
                                    width: `${clamp((compositeAqi / 500) * 100, 2, 100)}%`,
                                    background: `linear-gradient(90deg, ${COLORS.nominal}, ${compositeColor})`,
                                }}
                            />
                        </div>
                    </div>
                ...
                {/* ═══ CENTER — Spatial Operations Theater ═══ */}
                <div className="center-panel">
                    {/* ── FIX 2: Geographic Depth Layer ────────────────────────── */}
                    <div
                        className="absolute inset-0 opacity-20 pointer-events-none z-0"
                        style={{
                            backgroundImage: `
                              repeating-linear-gradient(
                                rgba(14, 165, 233, 0.07) 0px, 
                                rgba(14, 165, 233, 0.07) 1px, 
                                transparent 1px, 
                                transparent 48px
                              ),
                              repeating-linear-gradient(
                    ...
                    <svg
                        ...
                        // FIX-1: Add pointer-events-none to let hover pass through and zIndex
                        className="pointer-events-none z-10"
                        viewBox="0 0 100 100"
                        preserveAspectRatio="xMidYMid meet"
                        aria-label="Ward spatial operations map"
                    >
                        {/* Ward connections - mesh network */}
                        ...
                        {/* Sonar rings on Ward C */}
                        {(() => {
                            const wardC = wards.find((w) => w.id === "C");
                            return wardC ? (
                                <SonarRing cx={wardC.position.x} cy={wardC.position.y} />
                            ) : null;
                        })()}
                        ...
                        {wards.map((w) => {
                            ...
                            return (
                                <g key={w.id}>
                                    ...
                                    {/* ── CONTRAST 3: CENTRAL MAP — WARD C THREAT GLOW ────────────────────────────────── */}
                                    {isCritical && (
                                        <circle
                                            cx={w.position.x}
                                            cy={w.position.y}
                                    ...
                                    <circle
                                        ...
                                        style={{
                                            filter: isCritical ? 'drop-shadow(0 0 8px rgba(168, 85, 247, 0.9)) drop-shadow(0 0 20px rgba(168, 85, 247, 0.5))' : undefined,
                                        }}
                                    />
                                    {/* Label */}
                                    <text
                                        x={w.position.x}
                                        y={w.position.y - (isCritical ? 5 : 3)}
                                        textAnchor="middle"
                                        fill={isCritical ? COLORS.anomaly : "#94a3b8"}
                                        fontSize={isCritical ? "3" : "2.2"}
                                        fontFamily="'Bebas Neue', sans-serif"
                                        letterSpacing="0.5"
                                    >
                                        {w.name}
                    ...
                    {/* Anomaly Card (Glassmorphism) */}
                    {(() => {
                        const wardC = wards.find((w) => w.id === "C");
                        if (!wardC) return null;
                        return (
                            // ── FIX 1: Anomaly Card Layering ──────────────────────────
                            <div
                                style={{ position: "absolute", right: "12%", top: "28%", zIndex: 20 }}
                            >
                                {/* ── POLISH 4: CENTRAL MAP — ANOMALY CARD LEGIBILITY UPGRADE ────────────────────────────────────── */}
                                    ...
                                    <div style={{ marginTop: "10px" }}>
                                        {[
                                            ...
                                            >
                                                ...
                                                {/* ── CONTRAST 2: ANOMALY CARD — POLLUTANT BAR VALUE LEGIBILITY ────────────────────────────────── */}
                                                <span className="text-slate-100 font-bold font-mono text-xs flex-shrink-0 whitespace-nowrap">
                                                    {bar.value}
                                                </span>
                                            </div>
                                        ))}
                    ...
                    {/* SVG line connecting anomaly card to Ward C */}
                    <svg
                        style={{
                            position: "absolute",
                            inset: 0,
                    ...
                    >
                        ...
                        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                            {wards.map((w) => {
                                const statusColor =
                                    w.sensorStatus === "nominal"
                                        ? COLORS.nominal
                                        : w.sensorStatus === "warning"
                                            ? COLORS.warning
                                            : COLORS.danger;
                                return (
                                    <div
                ...
                {/* ═══ RIGHT PANEL — AI Response Engine ═══ */}
                <div className="right-panel">
                    ...
                    {/* QA-PATCH 3: RIGHT SIDEBAR — INTERVENTIONS HEADER LINE DISCIPLINE */}
                    ...
                    >
                        ...
                        <div className="criticality-bar">
                            <div
                                ...
                                style={{
                                    width: `${aiConfidence}%`,
                                    background: `linear-gradient(90deg, ${COLORS.rim}, ${COLORS.nominal})`,
                                }}
                            />
                        </div>
                    </div>
